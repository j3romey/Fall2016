 ...
 * listens for clients. When one connects a thread is spawned to deal with the
 * client.
 *
...
public class Server {
	...
	private RSATool RSA; // RSA encryption/decryption class
	...
	/**
	 * Called by a thread who's client has asked to exit. Gets rid of the
	 * thread.
	 * 
	 * @param st
	 *            The ServerThread to remove from the vector of active
	 *            connections.
	 */
	...
	public void killall() {
		 ...
		 * to stop blocking because of the IO operation, and check the shutdown
		 * flag. The thread will then exit itself when it sees shutdown is true.
		 * Then exits.
		 */
	...
	public void listen() {
		...
		while (!shutdown) {
			...
			try {
				 ...
				 * run() methods.
				 */
				st = new ServerThread(client, this, clientcounter++, RSA);
	 ...
	 * 
	 * @param args
	 *            First argument should be the port to listen on.
	 */
